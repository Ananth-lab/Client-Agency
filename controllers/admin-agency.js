        ...
        bcrypt.hash(req.body.password, cycles, async (err, hash) => {
            try {
                if(req.body.agid ){
                    ...
                    res.status(200).json({ newuser: new_client, message : "User added for the agency" });                    
                }
                else {
                    ...
                    const new_client = await client.save()
                    res.status(200).json({ newuser: new_client, agency : new_agency, message : "Admin and Agency created" });
                }
            }
            catch (error) {
                console.log(error)
