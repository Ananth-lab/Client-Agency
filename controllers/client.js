    ...
    try {
        ...
        if (user) {
            bcrypt.compare(req.body.password, user.password, (err, result) => {
                if (result == true) {
                    res.status(201).json({ message: "User login successful" ,userId : user._id, token : generateAccessToken(user.id)});
                }
                else {
...
exports.updateDetailsController = async (req,res, next) => {
    try {
        await User.findByIdAndUpdate(req.query.userId, {name : req.body.username, email : req.body.email, phone_number : req.body.phonenumber,total_bill : req.body.bill});
